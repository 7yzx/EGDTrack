...
parser.add_argument('--gt_path', type=str, help='path to the groundtruth data',
                    default='../../dataset/nyu_depth_v2/sync/')
...
parser.add_argument('--bn_no_track_stats', help='if set, will not track running stats in batch norm layers',
                    action='store_true')
parser.add_argument('--weight_decay', type=float, help='weight decay factor for optimization', default=1e-3)
...
parser.add_argument('--eval_summary_directory', type=str, help='output directory for eval summary,'
                    ...
                    default='./mymodels_edge_test_4/eval/')
...
if sys.argv.__len__() == 2:
    ...
    args.do_random_rotate = True
    args.do_online_eval = True
...
def compute_edges(gt, pred, mask):
    thre = 0.25
    gt[mask == 0] = 0
...
def sobel_operator(image):
    ...
    # 在图像上应用Sobel算子进行卷积操作
    gradient_x = torch_nn_func.conv2d(image, sobel_x.view(1, 1, 3, 3), padding=1)
    gradient_y = torch_nn_func.conv2d(image, sobel_y.view(1, 1, 3, 3), padding=1)
...
def online_eval(model, dataloader_eval, gpu, ngpus):
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        ...
        edge_measures = compute_edges(gt_depth,pred_depth,eval_mask)
        eval_measures[:9] += torch.tensor(measures).cuda(device=gpu)
        eval_measures[9] += 1
        eval_edge_measures[:3] += edge_measures
        eval_edge_measures[3] += 1
    ...
    if not args.multiprocessing_distributed or gpu == 0:
        ...
        edge_measures_cpu = eval_edge_measures
        edge_cnt = edge_measures_cpu[3]
        edge_measures_cpu /= edge_cnt
        ...
        print('Computing edge errors for {} eval samples'.format(int(edge_cnt)))
        print("{:>7}, {:>7}, {:>7}".format('Pv', 'Rv', 'Fv'))
        for i in range(3):
            print('{:7.3f}, '.format(edge_measures_cpu[i]), end='')
        print('{:7.3f}'.format(edge_measures_cpu[3]))
        return eval_measures_cpu,edge_measures_cpu
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    edge_criterion = edge_loss_test(0.9)
    # normal_criterion = loss_normal(0.5)
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            # print("first silog loss",loss)

            # edge_out = sobel_operator(depth_gt)

            # edge_depth = sobel_operator(depth_est)
            # 大概在3左右
            loss_g = edge_criterion.forward(image, depth_est, depth_gt, mask.to(torch.bool),global_step, writer=writer)
            print("loss grad", loss_g)
            # loss_n = normal_criterion.forward(edge_out, edge_depth, mask.to(torch.bool))
            # print("loss normal", loss_n)
            loss = loss_g + loss
            # print("loss_g",loss_g)
            # print("loss_n",loss_n)
            loss.backward()
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                    ...
                    writer.add_scalar('silog_grad_loss', loss_g, global_step)
                    # writer.add_scalar("loss normal", loss_n, global_step)
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded:
                time.sleep(0.1)
                model.eval()
                eval_measures, edge_measures = online_eval(model, dataloader_eval, gpu, ngpus_per_node)
                if eval_measures is not None:
                    for i in range(9):
                        if i<3:
                            eval_summary_writer.add_scalar(edge_metircs[i],edge_measures[i],int(global_step))
