...
def test_fewpic(args):
    ...
    # 图片所在文件夹路径
    data_dir = '../got10k/'
    got10k_depth_test_dir = '../'
    got10k_depth_dir = '../../../dataset_track/vot2022Show/'
    ...
    for i, sample in enumerate(test_loader):
        ...
        save_path = got10k_depth_test_dir + '/depth_10k/' + save_name + '_depth.png'
...
def show_coco_contrast(args,data_dir,output_dir,mode):
    """Test function."""
    args.mode = 'test'
    if mode == 'orin' or mode == None:
        args.checkpoint_path = '../models/bts_nyu_v2_pytorch_test/model'
        vis_name = '_visorin.png'
        print("generate depth with model {}".format(args.checkpoint_path.split('/')[-1]))
    elif mode == 'new':
        args.checkpoint_path = '../models/bts_nyu_v2_pytorch_test/model_rms_4371'
        vis_name = '_visnew.png'
        print("generate depth with model {}".format(args.checkpoint_path.split('/')[-1]))
    elif mode == 'compare':
        args.checkpoint_path = '../models/bts_nyu_v2_pytorch_test/model'
        new_checkpoint_path = '../models/bts_nyu_v2_pytorch_test/model_rms_4371'
        visorin_name = '_visorin.png'
        visnew_name = '_visnew.png'
        print("generate depth with model {} vs {}".format(args.checkpoint_path.split('/')[-1], new_checkpoint_path.split('/')[-1]))
    ...
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
    print("pic in {} stored in {}".format(data_dir,output_dir))
    ...
    for i, sample in enumerate(test_loader):
        ...
        if mode == 'compare':
            ...
            gc.collect()
            vis_path = os.path.join(output_dir,save_name + visorin_name)
            plt.imsave(vis_path,pred_depth,cmap='magma_r')

            args.checkpoint_path = new_checkpoint_path
            model = BtsModel(params=args)
            model = torch.nn.DataParallel(model)
            ...
            # 准备保存
            pred_depth = depth_est.detach().cpu().numpy().squeeze()
            vis_path = os.path.join(output_dir,save_name + visnew_name)
            plt.imsave(vis_path,pred_depth,cmap='magma_r')
        else:
            vis_path = os.path.join(output_dir,save_name + vis_name)
            plt.imsave(vis_path,pred_depth,cmap='magma_r')
...
if __name__ == '__main__':
    # test_fewpic(args)
    # show_coco_contrast(args,data_dir='../got10k',output_dir='../got10k_show',mode='new')
    show_coco_contrast(args,data_dir='../coco',output_dir='../coco_show',mode='orin')
    # show_coco_contrast(args,data_dir='../got10k',output_dir='../got10k_show',mode='new')