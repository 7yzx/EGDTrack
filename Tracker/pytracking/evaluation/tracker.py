...
class Tracker:
    ...
    args:
        ...
        run_id: The run id.
        display_name: Name to be displayed in the result plots.
        g_depth:weather to use generated depth map from rgb image
    """

    def __init__(self, name: str, parameter_dir:str , parameter_name: str, run_id: int = None, display_name: str = None, use_vot: bool=False):
        ...
        if self.run_id is None:
            ...
            if self.use_vot:
                self.results_dir = '{}/{}/{}/{}/{}/{}'.format(env.results_path, self.name,'cdtb',self.parameter_dir,
                                                           self.parameter_name, 'rgbd-unsupervised')
            else:
                self.results_dir = '{}/{}/{}'.format(env.results_path, self.name, self.parameter_name)
                self.segmentation_dir = '{}/{}/{}'.format(env.segmentation_path, self.name, self.parameter_name)
        ...
        self.visdom = None
        # if g_depth == True:
        #     print("use generated depth from rgb img")
    ...
    def get_parameters(self):
        """Get parameters."""
        if self.use_vot or self.parameter_dir is not None:
            param_module = importlib.import_module('pytracking.parameter.{}.{}.{}'.format(self.name, self.parameter_dir,self.parameter_name))
        else:
    ...
    def _read_image(self, image_file: str, dtype='color'):
        if isinstance(image_file, dict):
            ...
            elif dtype == 'myrgbcolormap':# 添加rgb+generatedepth
                img = get_rgbd_frame(image_file,None,dtype=dtype,depth_clip=True)
            else:
