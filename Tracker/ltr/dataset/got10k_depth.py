...
class Got10k_depth(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, dtype='depth', split=None, seq_ids=None, data_fraction=None):
        ...
        if split is not None:
            ...
            if split == 'train':
                ...
                file_path = os.path.join(ltr_path, 'data_specs', 'got10k_my_split.txt')
            elif split == 'val':
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        seq_path_color = seq_path
        seq_path_depth = seq_path.replace('color','depth')
        # seq_path_color = os.path.join( seq_path.replace(seq_name,'color'),seq_name)
        # seq_path_depth = os.path.join( seq_path.replace(seq_name,'depth'),seq_name)
        return (seq_path_color+'/{:08}.jpg'.format(frame_id+1),
                seq_path_depth+'/{:08}.png'.format(frame_id+1))    # frames start from 1
